...
import { BROWSER_HEADERS, MAX_BODY, TIMEOUT_MS, toHttps, fetchProtected, extractMetaImage, extractBiliPageCover, getBvid, } from '../functions/coverExtract.js';
import { imageCache, RateLimitError, UpstreamBlockedError, } from '../functions/rateLimit.js';
...
async function extractBilibiliCover(videoUrl) {
    ...
    if (!bvid)
        throw new Error('no bvid in url');
    // 1) Official API
    try {
        const api = await fetchProtected('bili-api:' + bvid, 'https://api.bilibili.com/x/web-interface/view?bvid=' + bvid, { referer: 'https://www.bilibili.com/' });
        if (api.status === 200 && api.text) {
            ...
            if (j?.code === 0 && typeof j.data?.pic === 'string' && j.data.pic)
                return toHttps(j.data.pic);
            if (j?.code === 0 && typeof j.data?.pages?.[0]?.first_frame === 'string' && j.data.pages[0].first_frame)
                return toHttps(j.data.pages[0].first_frame);
        }
    }
    catch (e) {
        if (e instanceof RateLimitError || e instanceof UpstreamBlockedError)
            throw e;
    }
    // 2) Video page HTML
    try {
        const page = await fetchProtected('bili-page:' + bvid, videoUrl, { referer: 'https://www.bilibili.com/' });
    ...
    // 3) Player API
    try {
        const api2 = await fetchProtected('bili-player:' + bvid, 'https://api.bilibili.com/x/player/pic?bvid=' + bvid, { referer: 'https://www.bilibili.com/video/' + bvid });
        if (api2.status === 200 && api2.text) {
            ...
            if (j?.code === 0 && typeof j.data === 'string' && j.data)
...
async function extractGenericCover(url) {
...
// ── Vite plugin ──────────────────────────────────────────────────────────────
export default function coverProxyPlugin() {
    return {
        name: 'cover-proxy',
